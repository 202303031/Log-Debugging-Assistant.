...
Features:
  ...
  - Manual Debugger tab (calls Ollama directly, no background agent needed)
...
def call_ollama(stack_trace: str) -> str:
    """Call the local Ollama LLM directly from the dashboard for manual debugging."""
    try:
...
*Powered by Ollama ({config.OLLAMA_MODEL}) + pgvector RAG*
"""
...
st.sidebar.caption(f"🤖 LLM: **Ollama ({config.OLLAMA_MODEL})**")
st.sidebar.caption(f"🌐 Ollama: **{config.OLLAMA_BASE_URL}**")
...
with tab2:
    ...
    st.write(
        "Paste a Java stack trace below. The local Ollama LLM will analyze it "
    ...
    if st.button("🚀 Analyze with Ollama AI", use_container_width=True, type="primary"):
        if manual_log.strip():
            with st.spinner("🤖 Ollama is analyzing your stack trace..."):
                analysis = call_ollama(manual_log.strip())
